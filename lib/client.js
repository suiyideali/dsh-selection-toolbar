...
.dyn-selpop .btw-answer .btw-table {
  width: 100%;
  max-width: 100%;
  margin: 2px 0 8px;
...
.dyn-selpop .btw-answer .btw-table th,
.dyn-selpop .btw-answer .btw-table td {
...
`
     ...
     *  The tag is REFRESHED on every load instead of skipping when one already
     *  exists: a stale tag left in the DOM by a bundle reload (HMR / plugin
     *  swap without a full page refresh) would otherwise keep the previous
     *  CSS_TEXT forever, silently dropping newer style rules. */
    function insertCss() {
      if (typeof document === 'undefined') return
      let style = document.querySelector('style[data-plugin-css=' + JSON.stringify(STYLE_TAG) + ']')
      if (!style) {
    ...
    // The popup is `position: fixed`. `left` was always clamped to the viewport
    // but `top` was not, so a selection anchored in the top ~140px that is also
    // TALLER than the space left below it (a big table, a long code block) got
    // `top = rect.bottom + 8` — i.e. rendered below the bottom edge, invisible.
    // That reads as "selecting text does nothing", and it hits short windows
    // (the desktop app) far more often than a maximised browser tab.
    // Height ≈ 3px container padding + 1px border each side + a 29px button row
    // (8 + 13 + 8) ≈ 38px; opening the custom input adds a ~48px row. Both
    // constants stay generous on purpose.
    const POPUP_GAP = 8
    const POPUP_RESERVED = 44
    const POPUP_RESERVED_CUSTOM = 92

    /**
     * Keep the popup box inside the viewport. `above` places the popup fully
     * over the selection (the renderer applies `translateY(-100%)`), so the
     * constraint is `top - reserved >= 4` there and `top + reserved <= vh - 4`
     * when it hangs below.
     */
    function clampPopupTop(top, above, reserved) {
      const vh = globalThis.innerHeight || 800
      return above
        ? Math.min(Math.max(top, reserved + 4), vh - 4)
        : Math.max(Math.min(top, vh - reserved - 4), 4)
    }
    ...
    // fenced code blocks, inline code, bold, headings-as-bold, lists and
    // markdown tables (GFM-style `| a | b |` blocks with a `---` separator). ----
    ...
    // then drop the empty edge cells produced by the surrounding pipes and
    // trim each cell.
    function btwSplitCells(line) {
      const cells = []
      let cur = ''
      let inCode = false
    ...
    // GFM separator row: every cell is only `-` and optional `:` (alignment).
    function btwIsTableSep(line) {
      const cells = btwSplitCells(line)
      return cells.length > 0 && cells.every((c) => /^:?-+:?$/.test(c))
    }

    function btwRenderTable(rows, keyBase) {
      const head = btwSplitCells(rows[0])
      const body = rows.slice(2).map(btwSplitCells)
      return React.createElement(
        'table',
        { key: keyBase, className: 'btw-table' },
        React.createElement(
          'thead',
          null,
          React.createElement(
            ...
            null,
            head.map((c, i) =>
              React.createElement('th', { key: keyBase + 'th' + i }, btwRenderInline(c, keyBase + 'th' + i))
            )
          )
        ),
        React.createElement(
          ...
          body.map((row, ri) =>
            React.createElement(
              ...
              { key: keyBase + 'r' + ri },
              row.map((c, ci) =>
                React.createElement('td', { key: keyBase + 'td' + ri + '_' + ci }, btwRenderInline(c, keyBase + 'td' + ri + '_' + ci))
              )
            )
          )
        )
    ...
    function btwRenderTextBlock(text, keyBase) {
      ...
      const flushTable = () => {
        ...
        const rows = table
        table = null
        // A table needs ≥2 rows and a GFM separator as the second row;
        // otherwise the pipe lines were prose — fall back to paragraphs.
        if (rows.length >= 2 && btwIsTableSep(rows[1])) {
          out.push(btwRenderTable(rows, keyBase + 't' + out.length))
          return
        }
        for (const row of rows) {
          out.push(
            React.createElement('p', { key: keyBase + 'p' + out.length, className: 'btw-p' },
              btwRenderInline(row, keyBase + 'p' + out.length)
            )
          )
        }
      }
      for (const raw of String(text).split('\n')) {
        ...
        // A candidate table row: contains a `|` and is neither a list item
        // nor a heading. GFM-style rows open with `|`; pipe-less rows
        // (header + separator without surrounding pipes) are buffered too —
        // the block only materializes as a table when a separator row shows
        // up, otherwise it falls back to plain paragraphs below.
        const heading = /^#{1,6}\s+(.*)$/.exec(trimmed)
        const isTableRow = !heading && trimmed.includes('|')
        if (isTableRow) {
          flushList()
          table = table || []
          table.push(line)
    ...
    // Resolve which session the toolbar acts on.
    //
    // `sessions.current` is authoritative when set, but it can be EMPTY while the
    // plugin renders in an overlay. Observed on the desktop app: every AI action
    // failed with 「找不到该会话的活跃实例」 (binding(undefined)), 「引用」 always
    // tripped the bridge mismatch because the dock had a real id while the popup
    // had none, and /btw returned 400 缺少 sessionId. The composer-dock slot hands
    // us the live session id directly, and that is the session the user is typing
    // in — so fall back to it instead of acting on an empty id.
    const isLiveSessionId = (v) => typeof v === 'string' && v.length > 0
    let warnedEmptyCurrentSession = false
    const resolveSessionId = (fromStore) => {
      if (isLiveSessionId(fromStore)) return fromStore
      const bridged = quoteBridge.sessionId
      if (isLiveSessionId(bridged) && !warnedEmptyCurrentSession) {
        warnedEmptyCurrentSession = true
        console.warn('[dsh-selection-toolbar] sessions.current is empty — falling back to the composer session id:', bridged)
      }
      return bridged
    }

    // ---- structured quote serialization -------------------------------------
    // Rendered content loses its markdown shape in the text layer: a <table>'s
    // cells come out tab-separated (no pipes at all) and a <pre> loses its fence,
    // so quoting `Selection.toString()` produced something that no longer renders
    // as a table / code block. These helpers serialize the SELECTED DOM instead.
    // `cloneContents()` keeps only the selected part of an element, so a partial
    // code-block or table selection stays partial.
    function quoteCell(cell) {
      // Escape pipes so one cell can never split into extra columns.
      return String(cell.textContent || '').replace(/\s+/g, ' ').trim().replace(/\|/g, '\\|')
    }
    ...
    function quoteTable(table) {
      ...
      for (const tr of table.querySelectorAll('tr')) {
        ...
        for (const cell of tr.children) {
          ...
          cells.push(quoteCell(cell))
        }
        if (!cells.length) continue
        if (header) hasHeaderCells = true
      ...
      const out = []
      // GFM always needs a header row. Use the table's own header when the markup
      // has <th> cells; otherwise emit an empty header so no data row is promoted.
      if (hasHeaderCells) {
        const head = rows[0].header ? rows[0] : rows.find((row) => row.header)
        out.push(lineOf(head.cells))
        out.push(lineOf(rule))
        for (const row of rows) {
          if (row !== head) out.push(lineOf(row.cells))
        }
      } else {
        out.push(lineOf(blanks))
        out.push(lineOf(rule))
        for (const row of rows) out.push(lineOf(row.cells))
      }
      return out.join('\n')
    }

    function quoteCode(pre) {
      const code = pre.querySelector('code')
      const raw = String((code || pre).textContent || '').replace(/\r\n/g, '\n').replace(/\n+$/, '')
      if (!raw.trim()) return ''
      let lang = ''
      if (code) {
        const found = String(code.className || '').match(/(?:language|lang)-([A-Za-z0-9_+#-]+)/)
        if (found) lang = found[1]
      }
      // Grow the fence when the body already contains backticks.
      let fence = '```'
      while (raw.indexOf(fence) >= 0) fence = fence + '`'
      return fence + lang + '\n' + raw + '\n' + fence
    }

    // Serialize a selection fragment in document order: <table> and <pre> become
    // markdown blocks, everything else stays plain text. Returns '' when the
    // fragment holds nothing structured, so prose quoting keeps its own style.
    function quoteMarkdownFromFragment(frag) {
      const chunks = []
      let sawStructured = false
      const walk = (node) => {
        for (const child of node.childNodes) {
          if (child.nodeType === 3) {
            chunks.push({ kind: 'text', text: child.data })
            continue
          }
          if (child.nodeType !== 1) continue
          if (child.tagName === 'TABLE') {
            sawStructured = true
            chunks.push({ kind: 'table', el: child })
            continue
          }
          if (child.tagName === 'PRE') {
            sawStructured = true
            chunks.push({ kind: 'code', el: child })
            continue
          }
          walk(child)
        }
      ...
      for (const chunk of chunks) {
        ...
        flush()
        const md = chunk.kind === 'table' ? quoteTable(chunk.el) : quoteCode(chunk.el)
    ...
    function quoteBlockOf(text, domStructured) {
      ...
      // 在文本层可能没有这些特征）。结构化内容必须逐行加 `> ` 前缀才仍是
      // 表格/fence，详见下面 structured 分支的说明。
      const structured = domStructured || lines.some((l) => /^\s*```/.test(l) || /^\s*\|.*\|\s*$/.test(l))
      if (structured) {
        // Prefix EVERY line. A table or a fence only survives as such when each of
        // its lines carries the quote marker: the previous "lazy blockquote" (first
        // line prefixed, the rest left bare) only works for plain paragraphs, so a
        // quoted table collapsed into raw text and a quoted fence lost its closing
        // marker (user-reported as 「划词回复表格渲染问题」). Empty lines become a
        // lone `>` so the blockquote — and any fence inside it — stays continuous.
        const quoted = []
        for (const line of lines) quoted.push(line.trim() ? '> ' + line : '>')
        while (quoted.length && quoted[0] === '>') quoted.shift()
        while (quoted.length && quoted[quoted.length - 1] === '>') quoted.pop()
        return quoted.join('\n') + '\n\n'
      }
      // Prose quote (default): only content lines carry the `> ` marker —
      // blank lines stay bare and runs of blank lines collapse to one, so a
      // multi-paragraph quote does not turn into a wall of lone '>' lines in
      // the composer (user feedback: every line got a prefix before). The
      // model still reads every '> '-prefixed line as quoted content, and the
      // bare blank line simply separates the quoted paragraphs.
      const out = []
    ...
    function SelectionPopup(props) {
      ...
      // see resolveSessionId: the overlay's sessions store can report no current
      // session even though the composer knows it
      const sessionId = resolveSessionId(useSessions((s) => s.current))
      ...
      // focus management: input → input phase, viewer → browsing, and the
      // console root during a live answer (so ↑ can open history browsing)
      React.useEffect(() => {
        if (!btwMode) return
        if (btwMode.view != null && btwViewerRef.current) {
          try { btwViewerRef.current.focus() } catch (e) {}
          return
        }
        if (btwMode.phase === 'input' && btwInputRef.current) {
          try { btwInputRef.current.focus() } catch (e) {}
          return
        }
        if (btwMode.phase === 'answer' && popupRef.current) {
        ...
        fetch(BTW_ROUTE, {
          ...
          .then((data) => {
            ...
            setBtwMode({
              ...
              answer,
              view: null,
              stats: data.contextEvents === undefined
                ? undefined // older host half without stats — omit the line rather than falsely warn "empty"
                : { events: Number(data.contextEvents) || 0, chars: Number(data.contextChars) || 0 }
            })
          })
          .catch((e) => {
      ...
      // execCommand fallback. Also used when the async clipboard API EXISTS but
      // rejects — Electron denies the clipboard permission in some setups, and
      // that used to surface as a bare 「操作失败」 without ever trying the
      // fallback (the old branch only ran when `navigator.clipboard` was absent).
      ...
      const copyTextToClipboard = (text) => {
        ...
        if (nav && nav.clipboard && nav.clipboard.writeText) {
          nav.clipboard.writeText(text).then(() => setFlash('copied')).catch((e) => {
            console.warn('[dsh-selection-toolbar] clipboard write rejected:', e)
            setFlash(copyViaExecCommand(text) ? 'copied' : 'error')
          })
        } else {
          setFlash(copyViaExecCommand(text) ? 'copied' : 'error')
        }
      ...
      React.useEffect(() => {
        ...
        const compute = () => {
          ...
          // `content-visibility: hidden` (the transcript virtualizes off-screen
          // message containers) makes Selection.toString() empty even though the
          // DOM text is right there — so don't bail yet; the range fragment
          // collected below doubles as a DOM-text fallback.
          ...
          try {
            ...
            // Only real block-level structure. Inline <code> has no line structure
            // of its own, so it must not push an otherwise prose selection into the
            // structured branch (which prefixes every line).
            structured = !!frag.querySelector('table, pre')
            // DOM-serialized form: a rendered table's text layer is tab-separated
            // (no pipes) and code loses its fence, so only the DOM can restore them.
            quote = quoteMarkdownFromFragment(frag)
            // DOM-text fallback: reached only when Selection.toString() was empty
            // (off-screen virtualization). Keeps tabs/newlines intact so the quote
            // path still sees the table/code structure.
            if (!text) text = (frag.textContent || '').trim()
          } catch (e) {}
          if (!text) return null
          ...
          // Keep the original preference (below for a selection near the top of the
          // viewport, above otherwise — the "above" branch always has room), but
          // flip sides when the chosen one cannot hold the popup. When NEITHER side
          // fits (a selection taller than the viewport) take the roomier side and
          // let the renderer clamp it inside the viewport.
          const roomBelow = vh - rect.bottom - POPUP_GAP
          const roomAbove = rect.top - POPUP_GAP
          let above = rect.top > 140
          if (above ? roomAbove < POPUP_RESERVED : roomBelow < POPUP_RESERVED) above = roomAbove > roomBelow
          return { text, quote, left, top: rect.top, bottom: rect.bottom, above, structured }
        }
        ...
        const onPointerDown = (e) => {
          ...
          if (popupRef.current && popupRef.current.contains(e.target)) {
            ...
            // start each in-popup interaction with a clean status: refresh() (the
            // only other place that clears it) is suppressed while interacting, so
            // a stale 「操作失败」 used to stick and make every button look broken.
            setFlash(null)
            return
          }
          dragging = true
      ...
      const runAction = (mode, customPrompt) => {
        ...
        if (submittingRef.current) {
          console.warn('[dsh-selection-toolbar] action ignored: a prompt is still in flight')
          return
        }
        const token = stateRef.current
        if (!token) {
          console.warn('[dsh-selection-toolbar] action ignored: no live selection state')
          return
        }
        ...
        promptSession(sessionId, promptFor(mode, text, customPrompt))
          .then((res) => {
            ...
            if (res && res.ok) {
              ...
              // keep the reason visible: the popup used to show a bare 「操作失败」
              // and the cause was swallowed, making this undiagnosable from a bug report
              console.warn('[dsh-selection-toolbar] action failed:', mode, (res && res.error) || res)
              setFlash('error')
            }
          })
          .catch((e) => {
            if (stateRef.current !== token) return
            console.warn('[dsh-selection-toolbar] action threw:', mode, e)
            setFlash('error')
          })
          .finally(() => {
      ...
      const runQuote = () => {
        ...
        if (!quoteBridge.actions || quoteBridge.sessionId !== sessionId) {
          ...
          console.warn('[dsh-selection-toolbar] quote bridge unavailable:', {
            hasActions: !!quoteBridge.actions,
            bridgeSessionId: quoteBridge.sessionId,
            sessionId
          })
          setFlash('error')
          return
        ...
        // Prefer the DOM-serialized form: a rendered table's text layer is
        // tab-separated, so only `state.quote` can produce a real GFM table.
        const block = quoteBlockOf(state.quote || text, state.structured)
        if (!block) {
          setFlash('error')
          return
        }
      ...
      const style = {
        left: state.left,
        // safety net: never render the popup outside the viewport — an invisible
        // popup is indistinguishable from "selecting text does nothing"
        top: clampPopupTop(
          above ? state.top - POPUP_GAP : state.bottom + POPUP_GAP,
          above,
          customOpen ? POPUP_RESERVED_CUSTOM : POPUP_RESERVED
        ),
      ...
      if (btwMode) {
        ...
        // Phase-dependent height: composing/pending states hug their content
        // (no dead space under a short history), while reading an answer or
        // browsing history locks the frame at 440×480 so entries of different
        // length never resize it mid-browsing. position: fixed + centering
        // means page scrolling never moves it either way.
        const consoleStyle = {
          position: 'fixed',
          left: '50%',
          top: '50%',
        ...
        const viewerBlock = viewed
          ? React.createElement('div', {
              ...
              React.createElement('div', { className: 'btw-pager' },
                ...
                React.createElement('span', { className: 'btw-pcount' }, (viewing + 1) + ' / ' + history.length),
                React.createElement('button', {
        ...
        return React.createElement(React.Fragment, null,
          ...
          React.createElement('div', {
            ...
            tabIndex: -1,
            onKeyDown: (e) => {
              // live answer: ↑ opens read-only history browsing (no stacked
              // history list below the answer — keyboard is the only path)
              if (btwMode.phase === 'answer' && viewing == null && e.key === 'ArrowUp' && history.length > 0) {
                e.preventDefault()
                e.stopPropagation()
                navBtwHistory(1)
              }
            ...
            onPointerDown: (e) => {
              // keep keyboard browsing alive: clicking non-interactive parts
              // refocuses the console root; the answer area is left alone so
              // manual text selection still works
              const t = e.target
              if (t && typeof t.closest === 'function' && !t.closest('button, input, textarea, .btw-answer')) {
                try { popupRef.current.focus() } catch (err) {}
              }
            },
            style: consoleStyle
          },
          ...
          btwMode.phase === 'answer' && viewing == null
            ...
            : null,
          btwMode.phase === 'answer' && viewing == null && btwMode.stats
            ? React.createElement('div', { className: 'btw-statline', key: 'stat' },
                btwMode.stats.events > 0
                  ? '上下文 · 已注入最近 ' + btwMode.stats.events + ' 条会话内容（' + btwMode.stats.chars + ' 字）' + (history.length > 0 ? ' · ↑ 翻历史' : '')
                  : React.createElement('span', { className: 'btw-warn' }, '上下文为空 · 本次仅基于划选内容作答' + (history.length > 0 ? ' · ↑ 翻历史' : ''))
              )
            : null,
          btwMode.phase === 'answer' && viewing == null
    ...
    function PasteQuoteSuggestion(props) {
      const useSessions = props.useSessions
      // same fallback as SelectionPopup: the overlay store may have no current session
      const sessionId = resolveSessionId(useSessions((s) => s.current))
