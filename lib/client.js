...
`
    ...
    function PasteQuoteSuggestion(props) {
      ...
      const convert = () => {
        ...
        // React 同步时机；比较前再做一次换行归一化（与 onPaste 对称）
        const doc = globalThis.document
        const area = doc && doc.querySelector('[data-composer-seat] textarea')
        ...
        // 校验粘贴区间仍未被用户改动；若已变则放弃（不覆盖用户输入）
        if (value.slice(s.start, s.end).replace(/\r\n/g, '\n') !== s.text) {
          setFlash('error')
          scheduleHide()
          return
        }
        ...
        const next = value.slice(0, s.start) + block + value.slice(s.end)
        try {
          quoteBridge.actions.setDraft(next)
        } catch (e) {
          setFlash('error')
        ...
        if (area) {
          try {
            area.focus()
            const pos = s.start + block.length
            area.setSelectionRange(pos, pos)
          } catch (e) {}
        }
        clearSug()
      ...
      return React.createElement('div', {
        ...
        React.createElement('span', { className: 'dyn-paste-sug-hint' }, flash === 'error' ? '无法转换：内容已被修改' : '转成 > 引用块')
      )
    }
