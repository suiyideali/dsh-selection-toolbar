...
`
    ...
    // reload. `delay` applies to the null → popup transition; `showCustom` hides
    // the 自定义 entry when off.
    const SETTINGS_KEY = 'dsh-selection-toolbar:settings'
    const SETTINGS_EVENT = 'dsh-selection-toolbar:settings-change'
    const SETTINGS_DEFAULTS = Object.freeze({ delay: 0, showCustom: true })
    ...
    function loadSettings() {
      ...
      try {
        ...
        if (typeof parsed.showCustom === 'boolean') base.showCustom = parsed.showCustom
      } catch (e) {}
      return base
    }
    ...
    function SelectionPopup(props) {
      ...
      return React.createElement('div', {
        ...
        React.createElement('div', { className: 'dyn-selpop-actions' },
          ...
          settingsRef.current.showCustom ? React.createElement('button', { type: 'button', className: 'dyn-selpop-btn', onClick: () => { setCustomOpen(!customOpen) } }, '自定义') : null
        ),
    ...
    // Renders one card in the plugin configuration section with the options the
    // popup reads live (delay + showCustom). Client-only persistence: writes go
    // straight to localStorage and dispatch SETTINGS_EVENT for the popup.
    function SettingsCard() {
      ...
      const row = { display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, padding: '8px 2px' }
      const title = { fontSize: 13, fontWeight: 600, color: 'var(--dsw-alias-label-primary, #111111)' }
      const sub = { fontSize: 11, color: 'var(--dsw-alias-label-secondary, #555555)', marginTop: 2 }
      const btn = {
        appearance: 'none', border: '1px solid var(--dsw-alias-border-l1, rgba(128,128,128,0.35))',
        borderRadius: 6, background: 'transparent', cursor: 'pointer',
        color: 'var(--dsw-alias-label-primary, #111111)', padding: '4px 12px', fontSize: 12
      }
      const btnOn = {
        ...btn, background: 'var(--dsw-alias-brand-primary, #4f8cff)',
        borderColor: 'transparent', color: '#ffffff'
      }
      return React.createElement('div', { className: 'dyn-selpop-settings' },
        React.createElement('div', { style: row },
          React.createElement('div', {},
            React.createElement('div', { style: title }, '划词工具栏'),
            React.createElement('div', { style: sub }, 'dsh-selection-toolbar — 划选文本时显示快捷工具栏')
          )
        ),
        React.createElement('div', { style: row },
          React.createElement('div', {},
            React.createElement('div', { style: title }, '弹窗出现延时'),
            React.createElement('div', { style: sub }, '选中文本后延迟多久弹出（毫秒）')
          ),
          React.createElement('input', {
          ...
          React.createElement('div', { style: { fontSize: 12, color: 'var(--dsw-alias-label-secondary, #555555)', minWidth: 38, textAlign: 'right' } }, settings.delay + ' ms')
        ),
        React.createElement('div', { style: row },
          React.createElement('div', {},
            React.createElement('div', { style: title }, '显示「自定义」按钮'),
            React.createElement('div', { style: sub }, '关闭后工具栏不再出现自定义问法入口')
          ),
          React.createElement('button', {
            type: 'button',
            'aria-pressed': settings.showCustom,
            onClick: () => update({ showCustom: !settings.showCustom }),
            style: settings.showCustom ? btnOn : btn
          }, settings.showCustom ? '开' : '关')
        ),
        React.createElement('div', { style: { ...row, justifyContent: 'flex-end' } },
          React.createElement('button', { type: 'button', onClick: reset, style: btn }, '恢复默认')
        )
      )
    }
    ...
    function apply(ctx) {
      ...
      slots.inject('settings.plugin.item', () => slots.register(
        { name: 'settings.plugin.item', id: 'selection-toolbar-settings', order: 30, label: '划词工具栏' },
