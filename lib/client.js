...
.dyn-selpop .btw-inputrow input:focus {
  border-color: var(--dsw-alias-brand-primary, #888888);
  background: var(--dsw-alias-bg-overlay, #ffffff);
}
.dyn-selpop .btw-hint {
...
`
    ...
    function SelectionPopup(props) {
      ...
      // /btw console state: null | {phase:'input'|'pending'|'answer', question?, answer?, error?}
      ...
      // anchor element captured when the console opens — scrolling re-anchors to it
      ...
      React.useEffect(() => {
        if (btwMode && btwMode.phase === 'input' && btwInputRef.current) {
          try { btwInputRef.current.focus() } catch (e) {}
        }
      }, [btwMode])

      // ---- /btw console helpers ----
      // Capture a bounded anchor block for the live selection BEFORE the input
      // takes focus and collapses it; inline nodes can span many lines, so walk
      // up (max 4 levels) to the first element with a reasonably sized box.
      const captureAnchor = () => {
        ...
        for (let i = 0; node && i < 4; i++) {
          ...
          if (rect && rect.height > 0 && rect.height <= 160 && rect.width >= 40) break
          node = node.parentElement
        }
        anchorRef.current = node || el
      }
      ...
      const submitBtw = (questionRaw) => {
        ...
        setBtwMode({ phase: 'pending', question })
        fetch(BTW_ROUTE, {
          method: 'POST',
          ...
          .then((answer) => {
            ...
            setBtwMode({ phase: 'answer', question, answer })
          })
          .catch((e) => {
      ...
      React.useEffect(() => {
        ...
        // /btw console: scrolling re-anchors the popup to the captured anchor
        // block instead of closing it — the answer must survive reading around.
        // When the anchor leaves the document, the console closes (the answer
        // is already saved to the session's side-question history).
        const repositionFromAnchor = () => {
          ...
          let rect
          try { rect = anchor.getBoundingClientRect() } catch (e) { return }
          ...
          setState((prev) => (prev ? { ...prev, left, top: rect.top, bottom: rect.bottom, above } : prev))
        }
        const onPopupMove = () => {
          if (pendingShowRef.current) {
            pendingShowRef.current()
          ...
          if (btwModeRef.current) repositionFromAnchor()
          else onHide()
        }
      ...
      // ---- /btw console: the answer lives inside this popup ----
      if (btwMode) {
        const consoleStyle = { ...style, minWidth: 280, width: 420, maxWidth: 'calc(100vw - 20px)' }
        const history = loadBtwHistory(sessionId)
        const historyLine = (entry, key) =>
          React.createElement('div', { key }, 'Q ' + entry.q.slice(0, 48) + ' / A ' + entry.a.slice(0, 64))
        return React.createElement('div', {
          ...
          React.createElement('div', { className: 'btw-head' },
            React.createElement('span', null, '顺便问 · 不进入对话'),
          ...
          btwMode.phase === 'input' && history.length > 0
            ? React.createElement('div', { className: 'btw-history' },
                history.slice(0, 2).map((entry, idx) => historyLine(entry, 'h' + idx)),
                history.length > 2 ? React.createElement('div', { key: 'more' }, '还有 ' + (history.length - 2) + ' 条更早') : null
              )
            : null,
          btwMode.phase === 'input'
            ...
            : null,
          btwMode.phase === 'input' && btwMode.error
            ? React.createElement('div', { className: 'btw-error' }, btwMode.error)
            : null,
          btwMode.phase === 'pending'
            ? React.createElement('div', { className: 'btw-hint' }, '正在基于当前会话内容思考…（不进入对话，无工具）')
            : null,
          btwMode.phase === 'answer'
            ? React.createElement('div', { className: 'btw-q' }, 'Q ' + btwMode.question)
            : null,
          btwMode.phase === 'answer'
            ? React.createElement('div', { className: 'btw-answer' }, btwMode.answer)
            : null,
          btwMode.phase === 'answer' && history.length > 1
            ? React.createElement('div', { className: 'btw-history' },
                history.slice(1, 3).map((entry, idx) => historyLine(entry, 'o' + idx)),
                history.length > 3 ? React.createElement('div', { key: 'more' }, '还有 ' + (history.length - 3) + ' 条更早') : null
              )
            : null,
          btwMode.phase === 'answer'
            ? React.createElement('div', { className: 'btw-actions' },
                React.createElement('button', { type: 'button', className: 'btw-btn', onClick: () => copyTextToClipboard(btwMode.answer) }, '复制'),
                React.createElement('button', {
                  type: 'button',
                  className: 'btw-btn',
            ...
            : null,
          btwMode.phase !== 'answer'
            ? React.createElement('div', { className: 'btw-hint' }, '仅基于当前会话内容 · 不使用工具 · 不进入对话')
            : null,
