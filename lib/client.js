...
`
    ...
    function SelectionPopup(props) {
      ...
      if (btwMode) {
        const consoleStyle = { ...style, minWidth: 300, width: 440, maxWidth: 'calc(100vw - 20px)' }
        ...
        const historyBlock = (entries, moreCount, key) =>
          entries.length || moreCount > 0
            ? React.createElement('div', { className: 'btw-history', key },
                entries.map(historyLine),
                moreCount > 0 ? React.createElement('div', { className: 'btw-more' }, '还有 ' + moreCount + ' 条更早') : null
              )
            : null
        return React.createElement('div', {
          ...
          btwMode.phase === 'input' ? historyBlock(history.slice(0, 2), Math.max(0, history.length - 2), 'h') : null,
          ...
          btwMode.phase === 'input'
            ...
            : null,
          btwMode.phase === 'input'
            ? React.createElement('div', { className: 'btw-foot', key: 'ft' }, '仅基于当前会话内容作答 · Enter 发送')
            : null,
          ...
          btwMode.phase === 'answer'
            ...
            : null,
          btwMode.phase === 'answer' ? historyBlock(history.slice(1, 3), Math.max(0, history.length - 3), 'o') : null,
          btwMode.phase === 'answer'
