...
`
    ...
    // hides individual toolbar entries (复制/引用/询问/解释/翻译/总结/自定义).
    // `showCustom` is kept as a legacy mirror of the custom entry and is
    // migrated into hiddenActions on load so older storage keeps working.
    ...
    const SETTINGS_DEFAULTS = Object.freeze({ delay: 0, showCustom: true, hiddenActions: [] })
    ...
    function loadSettings() {
      ...
      try {
        ...
        // migrate the legacy showCustom flag into the list
        if (typeof parsed.showCustom === 'boolean' && !parsed.showCustom && !base.hiddenActions.includes('custom')) {
          base.hiddenActions = base.hiddenActions.concat('custom')
        }
      } catch (e) {}
      return base
    }
    ...
    function saveSettings(next) {
      const payload = { ...next, showCustom: !next.hiddenActions.includes('custom') }
      try {
        const store = globalThis.localStorage
        if (store) store.setItem(SETTINGS_KEY, JSON.stringify(payload))
      } catch (e) {}
      try {
      ...
      return React.createElement('div', {
        ...
        React.createElement('div', { className: 'dyn-selpop-actions' },
          visibleActions.map((def) => {
            ...
            if (def.id === 'custom') return React.createElement('button', { key: def.id, type: 'button', className: 'dyn-selpop-btn', onClick: () => { setCustomOpen(!customOpen) } }, def.label)
            return React.createElement('button', { key: def.id, type: 'button', className: 'dyn-selpop-btn', onClick: () => runAction(def.id) }, def.label)
          })
        ),
        customOpen && !isHidden(settingsRef.current, 'custom') ? React.createElement('div', { className: 'dyn-selpop-custom' },
